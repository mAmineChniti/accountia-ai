...
class AccountingEngine:
    ...
    async def process_period(
        ...
        journal_entries = await self._generate_journal_entries(invoices, product_cost_map)
        task.journal_entries = journal_entries

        # 4. Calculate Tunisian taxes
        tax_calculations = await self._calculate_taxes(invoices, products, task.period_start, task.period_end)
        ...
        task.financial_summary = self._calculate_financial_summary(invoices, journal_entries)

        # 6. Generate reports
        task.reports = await self._generate_reports(invoices, journal_entries, task.period_start, task.period_end)

        # 7. AI analysis and insights
        ai_result = await self._generate_ai_analysis(invoices, journal_entries, task.financial_summary)
                ...
                if tax_amount > 0:
                    entries.append(
                        JournalEntry(
                            ...
                            debit=Decimal("0"),
                            credit=tax_amount,
                            description=f"VAT liability for invoice {inv_number}",
                            invoice_id=inv_id,
                        )
                    )
