...
class TunisianTaxService:
    ...
    def calculate_vat(
        self,
        invoices: list[dict],
        product_category_map: dict[str, str] | None = None,
    ) -> dict[str, Decimal]:
        """
    ...
    def calculate_corporate_income_tax(
        ...
        rate = self.CORPORATE_SME_RATE if is_sme or annual_revenue <= sme_threshold else self.CORPORATE_STANDARD_RATE
    ...
    def calculate_period_taxes(
        ...
        taxable_revenue = sum(
            Decimal(str(inv.get("totalAmount", 0))) for inv in invoices if inv.get("status") not in ["DRAFT", "VOIDED"]
        )
        ...
        for inv in invoices:
            for item in inv.get("lineItems", []):
                ...
                product_id = str(item.get("productId", ""))
                product = next((p for p in products if str(p.get("_id")) == product_id), None)
                if product:
