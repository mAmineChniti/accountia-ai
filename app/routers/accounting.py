...
from app.db.mongodb import get_platform_db, get_tenant_db, sanitize_bson_types
...
class CreateAccountingJobResponse(BaseModel):
    ...
    estimated_completion: datetime | None = Field(
        None, alias="estimatedCompletion", description="Estimated completion timestamp (UTC)"
    )
...
class AccountingJobStatusResponse(BaseModel):
    ...
    started_at: datetime | None = Field(None, alias="startedAt", description="When processing started")
    completed_at: datetime | None = Field(None, alias="completedAt", description="When processing completed")
    error_message: str | None = Field(None, alias="errorMessage", description="Error message if failed")
    journal_entries_count: int = Field(
        0, alias="journalEntriesCount", description="Number of journal entries generated"
    )
    reports_generated: int = Field(0, alias="reportsGenerated", description="Number of reports generated")
    # Time estimates
    estimated_seconds: int | None = Field(
        None, alias="estimatedSeconds", description="Estimated total runtime in seconds"
    )
    estimated_completion: datetime | None = Field(
        None, alias="estimatedCompletion", description="Estimated completion timestamp (UTC)"
    )
    estimated_time_remaining: int | None = Field(
        None, alias="estimatedTimeRemaining", description="Estimated remaining time in seconds"
    )
    model_config = {"populate_by_name": True}
    ...
    resp = AccountingJobStatusResponse(
        ...
        estimated_time_remaining=(
            max(
                0,
                int(
                    task_data.get("estimated_seconds", 0)
                    - ((datetime.utcnow() - task_data.get("started_at")).total_seconds())
                ),
            )
            if task_data.get("status") in ["processing", "pending"]
            and task_data.get("estimated_seconds")
            and task_data.get("started_at")
            else None
        ),
    )
...
async def cancel_accounting_job(
    ...
    if current_status not in [AccountingTaskStatus.PENDING, AccountingTaskStatus.PROCESSING]:
        raise HTTPException(
            status_code=400,
            detail=(
                f"Cannot cancel job with status '{current_status}'. Only pending or processing jobs can be cancelled."
            ),
        )
...
async def get_accounting_history(
    ...
    tenant_db = get_tenant_db(database_name)
    cursor = tenant_db["accounting_tasks"].find({"business_id": business_id}).sort("created_at", -1).limit(limit)
...
@router.get(
    ...
    description=(
        "Comprehensive work log showing everything the AI Accountant has done for a business. "
        "Includes detailed period info, journal entry counts, tax calculations, and financial summaries."
    ),
...
async def get_all_accountant_work(
    ...
    total_revenue = sum(
        float(t.get("financial_summary", {}).get("total_revenue", 0)) for t in tasks if t.get("status") == "completed"
    )

    total_journal_entries = sum(len(t.get("journal_entries", [])) for t in tasks)
    ...
    return {
        ...
        "summary": {
            ...
            "totalInvoicesProcessed": total_invoices_processed,
        },
        "accountingPeriods": [
            {
                "taskId": t["task_id"],
...
@router.get(
    ...
    description=(
        "Calculate taxes per Tunisian law: VAT (19%, 13%, 7%), Corporate Tax (IS), "
        "and Withholding Tax. Returns annual summary with monthly breakdowns and tax calendar."
    ),
...
async def get_tunisian_tax_summary(
    ...
    if not summary_doc:
        raise HTTPException(
            status_code=404,
            detail=(
                f"Tax summary for year {year} not found. "
                f"POST /business/{business_id}/taxes/calculate?year={year} to compute it."
            ),
        )
    ...
    return {
        ...
        "taxCalendar": [
            {
                "period": d.get("period"),
                "dueDate": (d.get("vat_due_date") or d.get("due_date")).isoformat()
                if (d.get("vat_due_date") or d.get("due_date"))
                else None,
...
async def calculate_tunisian_tax_summary(
    ...
    for month in range(1, 13):
        ...
        month_invoices = [
            inv for inv in invoices if month_start <= inv.get("issuedDate", datetime.utcnow()) <= month_end
        ]
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to persist tax summary") from e
